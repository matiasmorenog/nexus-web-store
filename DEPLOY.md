...
- [x] Ignored Build Step en **vape-store**: `bash scripts/vercel-should-build-app2.sh`
...
| `vape-store` | VAPORX (app2) | https://vape-store.vercel.app | `vape-demo` |
...
| `AUTH_URL` | `https://goat-indumentaria.vercel.app` | `https://vape-store.vercel.app` |
...
- App2: `https://vape-store.vercel.app/api/auth/callback/google` (local app2: `http://localhost:3001/api/auth/callback/google`)
...
3. En **Vercel** (cada proyecto: `goat-indumentaria` y `vape-store`):
...
- App2: `https://vape-store.vercel.app/api/webhooks/mercadopago`
...
**Prioridad de preview en PRs:** `goat-indumentaria` (app1) es el build principal (demo más completa hoy); `vape-store` (app2) es complementario. Podés probar app2 con `npm run dev:app2` cuando el Ignored Build Step lo saltee. Ver `docs/ci.md`.
