...
export default async function CheckoutPage() {
  ...
  const pickupOnly = config.features.pickupOnly;
  const couponsEnabled = await storeHasModule(store.id, "coupons");
  const dynamicShippingEnabled =
    !pickupOnly && (await isCarrierShippingEnabled(store.id));
  ...
  return (
    <CheckoutView
      ...
      allowPickup={store.allowPickup || pickupOnly}
