...
import { saveStoreSeoSettings } from "@/lib/seo/admin-persist";
...
export async function GET() {
  ...
  const settings = await getStoreSeoSettingsForAdmin(authResult.storeId);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const body = (await request.json()) as StoreSeoSettingsData;
    await saveStoreSeoSettings(authResult.storeId, body);
