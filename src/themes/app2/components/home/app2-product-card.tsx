...
export type App2ProductCardData = StorefrontProductCard & {
  categoryLabel: string;
};

type App2ProductCardProps = App2ProductCardData & {
  badge?: string | null;
...
function resolveBadge(product: App2ProductCardData): {
  label: string | null;
  className: string;
} {
  if (product.promo2x1) {
...
export function App2ProductCard(props: App2ProductCardProps) {
  const {
    slug,
    name,
    imageUrl,
    ...
    <article className="group overflow-hidden rounded-lg border border-app2 bg-app2-card transition-all duration-300 hover:border-[color-mix(in_srgb,var(--brand-primary)_30%,transparent)]">
      <div className="relative aspect-[4/5] overflow-hidden bg-[var(--brand-primary-mid)]">
        {imageUrl ? (
          ...
          <div className="flex h-full items-center justify-center text-xs text-app2-muted">Sin imagen</div>
        )}
      ...
      <div className="p-3 sm:p-4">
        <p className="mb-1 text-[10px] font-medium uppercase tracking-widest text-app2-muted">
          {categoryLabel}
        </p>
